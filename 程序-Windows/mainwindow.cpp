...
void MainWindow::onLoadFile()
{
    ...
    // 检查路径是否存在
    if (QDir(dijkstraPath).exists())
    {
        defaultPath = dijkstraPath;
    }
    else
    {
        // 如果找不到默认路径，使用当前工作目录或用户文档目录
...
void MainWindow::ensureDatabaseLoaded()
{
    ...
    // Windows: 使用AppData\Local\DijkstraApp
    // macOS: 使用~/Library/Application Support/DijkstraApp
    QString dataDir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    if (dataDir.isEmpty())
    {
        // 备用方案：使用用户目录下的.dijkstra_app文件夹
        dataDir = QDir::homePath() + QDir::separator() + ".dijkstra_app";
    }
    QDir dir(dataDir);
    if (!dir.exists())
        dir.mkpath(".");
